...
class Game extends Component {
  ...
  async fetchParticipants() {
    ...
    Pong.socket.getPastEvents('NewParticipant', eventFilterConfig, async (err) => {
      if (err) {
  ...
  handleNewParticipant(data) {
    if (data.returnValues.gameIndex === this.props.match.params.gameIndex) {
      const arrayName = parseInt(data.returnValues.participantIndex, 10) % 2 === 0 ? 'teamAParticipants' : 'teamBParticipants';
      const participants = this.state[arrayName];
  ...
  renderParticipantList(listName) {
    return (
      <div>
        {
          this.state[listName].map((participant, i) => (
            <div className="participant-info" key={i}>
              ...
              <div className="participant-addr">
                {participant.user}
              </div>
            </div>
          ))
    ...
    return (
      <div className="container">
        <div className="join-game-form">
          <div className="join-game-label">
Your name:
          </div>
          <input value={this.state.newParticipantName} onChange={this.handleNewParticipantNameChanged.bind(this)} />
          <div className="btn btn-info" onClick={this.handleJoinGameClicked.bind(this)}>
Join
          </div>
          {this.state.privateKey.length && (
          <div className="btn btn-danger" onClick={this.handleLeaveGameClicked.bind(this)}>
Leave
          </div>
          )}
        </div>
        ...
        <div className="row">
          <div className="col-xs-6 text-center participant-list-title">
Team A:
            {this.state.teamA}
          </div>
          <div className="col-xs-6 text-center participant-list-title">
